...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Always read role from profiles table (source of truth)
      ...
      const role = profile?.role || result.data.user?.user_metadata?.role || 'buyer';
  ...
  return (
    <div className="min-h-screen dark:bg-dark bg-light flex flex-col px-6 py-12 safe-top">
      <button onClick={() => navigate('/')} className="mb-8 p-2 rounded-xl dark:text-dark-text text-light-text self-start">
      ...
      <div className="flex-1 flex flex-col justify-center max-w-sm mx-auto w-full">
        <div className="mb-8">
          ...
          <h1 className="text-2xl font-bold dark:text-dark-text text-light-text">Welcome back</h1>
          <p className="text-sm dark:text-dark-muted text-light-muted mt-1">Sign in to your SELLIZI account</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              ...
              />
              <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 dark:text-dark-muted text-light-muted">
                {showPassword ? (
          ...
          <button onClick={() => navigate('/forgot-password')} className="text-xs text-primary font-medium">
            Forgot password?
          </button>
